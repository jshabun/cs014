...
bool Graph::add_vertex(const string& v) {
  ...
  if (names_.find(v) != names_.end()) {
    return false;
  }

  if(!isValidVertexName(v)) {
    return false;
  }
...
bool Graph::add_edge(const string& v1, const string& v2, unsigned int cost) {
  ...
  if (!isValidVertexName(v1)) {
    return false;
  }

  if (!isValidVertexName(v2)) {
    return false;
  }

  if (names_.find(v1) == names_.end()) {
    return false;
  }

  if (names_.find(v2) == names_.end()) {
    return false;
  }

  size_t vert1 = names_.find(v1)->second;
  size_t vert2 = names_.find(v2)->second;

  Vertex &currVert = vertices_.at(vert1); 

  currVert.neighbors.emplace_back(vert2, cost);
  ...
  while(!queue.empty()) {
    ...
    for(auto it : curr.neighbors) {
      if(visited.at(it.first) == false) {
        ...
        if(curr.distance + it.second < vertices_.at(it.first).distance) {
